...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/04 08:29:11 by adiaz-do          #+#    #+#             */
/*   Updated: 2021/05/10 14:21:17 by adiaz-do         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_file(t_data *vars, int args_n, char **args)
{
	...
	if (args_n < 2 || args_n > 3 || (args_n == 3 && ft_strncmp(args[2],
				 "--save\0", 7) != 0))
		return (0);
	initialize_vars(vars), fd = open(args[1], O_RDONLY);
	if (fd == -1)
		return (0);
